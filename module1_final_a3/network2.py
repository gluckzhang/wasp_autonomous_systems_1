# a 2-layer neural network with momentum training to classify cifar10 images use the tensorflow library
...
# this will be defined in add_layer function
...
hidden_nodes = 100
l1 = add_layer(x_input, input_dim, hidden_nodes, activation_function=tf.nn.relu)
y = add_layer(l1, hidden_nodes, 10, activation_function=tf.nn.softmax)
...
# Use momentum optimizer with a learning rate of .001
learning_rate = .001
train_step = tf.train.MomentumOptimizer(learning_rate,0.99).minimize(cross_entropy)
...
with tf.Session() as sess:
    ...
    if using_tensorboard:
        ...
        summary_writer = tf.summary.FileWriter('tensorboard/network2', sess.graph)
    ##################################################
